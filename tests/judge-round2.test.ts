...
describe("A. approvals that survive a relevant change", () => {
  ...
  it("A06. an element in an approved request that receives a recheck can be reviewed on its own, and re-approving the request does not do it", () => {
    ...
    expect(isReviewed(find(confirmed, "xcheck:days")!, confirmed.request)).toBe(true);
...
describe("B. verification that still confuses word presence with support of a fact", () => {
  it("B01. \"Thursdays don't work for us\" structured as a day that works is withheld: the apostrophe keeps its negation", () => {
    ...
    expect(d.recordings[0].claims.some((c) => c.key === "days_that_work")).toBe(false);
    expect(d.withheld).toEqual([expect.objectContaining({ key: "days_that_work", reason: "negation_mismatch" })]);
  });

  it("B02. quoting only \"Thursdays\" out of \"Not Thursdays\" does not make Thursday a day that works: the source clause is checked", () => {
    const d = recordingDraft(HAND_TRANSCRIPT, [claim({ key: "days_that_work", label: "Days that work", quote: "Thursdays", days: ["thursday"] })]);
    expect(d.recordings[0].claims.some((c) => c.key === "days_that_work")).toBe(false);
    expect(d.withheld).toEqual([expect.objectContaining({ key: "days_that_work", reason: "negation_mismatch" })]);
  });

  it("B03. \"after 3 am\" with earliestHour 15 is not 15:00, and never agrees with a form that says after 3 pm", () => {
    ...
    expect(d.recordings[0].claims.some((c) => c.key === "time_window" && c.earliestHour === 15)).toBe(false);
    ...
    expect(find(s, "xcheck:time")?.finding ?? "absent").not.toBe("consistent");
  });

  it("B04. \"before 3 pm\" with earliestHour 15 does not agree with a form that says after 3 pm", () => {
    ...
    const s = buildReview(submission(), d, NOW);
    expect(find(s, "xcheck:time")?.finding ?? "absent").not.toBe("consistent");
    expect(find(s, "xcheck:time")?.criterion?.result ?? "absent").not.toBe("met");
  });
  ...
  it("B06. in the recorded review as delivered, the hour the model says it assumed does not become a consistent cross-check", () => {
  ...
  it("B07. \"We cannot do the sessions at home\" with a structured place home is withheld, and never agrees with a form that says home", () => {
    ...
    expect(d.recordings[0].claims.some((c) => c.key === "location_preference")).toBe(false);
    expect(d.withheld).toEqual([expect.objectContaining({ key: "location_preference", reason: "negation_mismatch" })]);
    const sub = submission();
    sub.answers.location = "home";
    const s = buildReview(sub, d, NOW);
    expect(find(s, "xcheck:location")?.finding ?? "absent").not.toBe("consistent");
  });
  ...
  it("B13. \"after 3:00\" with earliestHour 12 is withheld: the 00 of the minutes is not an hour", () => {
...
describe("C. rejections that disappear, documents the extraction omitted, Spanish that attributes an assumption", () => {
  ...
  it("C05. the Spanish message composed from the delivered review does not attribute \"a partir de las 15:00\" to the parent: the hour was assumed", () => {
    ...
    expect(text).not.toMatch(/a partir de las/);
    expect(text).toMatch(/¿podría confirmar el momento del día que le conviene\?/);
  });
});
